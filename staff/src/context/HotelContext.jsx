...
export const HotelProvider = ({ children }) => {
  ...
  const fetchDiningTables = async () => {
    try {
      const { data } = await axios.get(`${API_URL}/dining`);
  ...
  const fetchDiningReservations = async () => {
    try {
      const { data } = await axios.get(`${API_URL}/dining/reservations`, getHeader());
      setDiningReservations(data);
    } catch (error) {
      console.error('Fetch dining reservations error:', error);
    }
  };

  const updateDiningStatus = async (id, status) => {
    try {
      await axios.put(`${API_URL}/dining/reservations/${id}/status`, { status }, getHeader());
